...
//Componente widget per mostrare un riepilogo delle perdite per un numero di mesi scelto dall'utente (3,6,12 mesi)
const WidgetPerdite = () => {
    //Riferimento alla funzione per navigare verso la pagina di dettaglio delle perdite
    const navigate = useNavigate();
    //Stato per memorizzare le perdite 
    ...
    //Calcolo del valore totale delle perdite
    let valoreTotale = 0;
    perdite.forEach(item => {
        valoreTotale += item.valorePerdita;
    });
    ...
    //All'avvio del componente viene richiamata la funzione fetchDataPerdite per ottenere i dati delle perdite
    ...
    useEffect(() => {
        const fetchDataPerdite = async () => {
          try {
            ...
            const resultPerdite = await getData(`/api/data/perdite?startDate=${numeroMesiIndietro}&endDate=${oggi}`);
            //Sorting dei dati ricevuti per data dal piu recente al meno recente
            resultPerdite.sort((a, b) => new Date(b.data) - new Date(a.data));
            //Imposto i dati delle perdite
            setPerdite(resultPerdite);
          } catch (errore) {
            console.error('Errore caricamento dati perdite:', errore);
          }
        };
        //Eseguo effettivamente le operazioni
        fetchDataPerdite();
      }, [numeroMesi]);
    //Funzione per navigare alla pagina del dettaglio perdite
    function handleClick() {
        navigate("/perdite");
    }

    return (
        /* Imposto la tabella del riepilogo perdite */
            ...
            <div className='border border-secondary bg-light d-flex flex-column h-100' style={{ overflowY: 'auto', overflowX: 'hidden', maxHeight: 'calc(100vh - 585px)' }}>
                ...
                <div className='row clickable' onClick={handleClick}>
                    <div className='col-8 d-flex justify-content-center'>
                        <span className='fs-4'>Perdite Prodotti ultimi {numeroMesi} mesi</span>
                    </div>
                    <div className='col-3 d-flex justify-content-center'>
                        <span className='fs-4 text-danger'>-{(valoreTotale).toLocaleString('it-IT', { style: 'currency', currency: 'EUR' })}</span>
                    </div>
                </div>
                {/* Appendo la tabella delle perdite */}
                <div className='flex-grow-1'>
                    <table className="m-0 table table-bordered" >
                        ...
                        <tbody>
                            {/* Per ogni perdita creo una riga di tabella e le sue celle */}
                            {perdite.map((item, index) => {
                                ...
                                return (
                                    <tr key={item.id}>
                                        ...
                                        {/* Il valore perdita dell'entry viene trasformato in tipologia monetaria con stile italiano */}
